...
@section('css')
    <link rel="stylesheet" href="{{ asset('css/admin/shop_reservations.css') }}">
@endsection
                ...
                <tbody>
                    @forelse ($users as $user)
                        <tr class="reservations__data">
                            ...
                            <td>{{ $user->name }}</td>
                            <td>{{ $user->email }}</td>
                            <td>{{ (new Carbon\Carbon($user->pivot->date))->isoFormat('Y年MM月DD日(ddd)') }} /
                                {{ (new Carbon\Carbon($user->pivot->time))->format('H:i') }}</td>
